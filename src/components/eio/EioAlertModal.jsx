    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm px-4">
      <div className="relative flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div>
              ...
              <ul className="mt-2 space-y-2">
                {alert.insights.map((insight) => (
                  <li key={insight} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700">
                    {insight}
                  </li>
                ))}
              </ul>
              ...
              <ul className="mt-3 space-y-2 text-sm text-gray-600">
                {alert.detectedAt && (
                  <li className="flex items-center gap-2">
                    ...
                    {translate('dashboard.eio.detail.detectedAt', {
                ...
                {alert.updatedAt && (
                  <li className="flex items-center gap-2">
                    ...
                    {translate('dashboard.eio.detail.updatedAt', {
                ...
                {alert.resolvedAt && (
                  <li className="flex items-center gap-2">
                    ...
                    {translate('dashboard.eio.detail.resolvedAt', {
              ...
              <div className="mt-3 space-y-2 text-sm text-gray-600">
                <p>
                  <span className="font-medium text-gray-700">
                    {translate('dashboard.eio.detail.assignedTo')}:
                  </span>{' '}
                  {alert.actionTakenBy || translate('dashboard.eio.detail.unassigned')}
                </p>
                <p>
                  <span className="font-medium text-gray-700">
                    {translate('dashboard.eio.detail.language')}:
                  </span>{' '}
                  {alert.language || '—'}
                </p>
                <p>
                  <span className="font-medium text-gray-700">
                    {translate('dashboard.eio.detail.tokensUsed')}:
                  </span>{' '}
                  {alert.tokensUsed ?? '—'}
                </p>
              </div>
        ...
        <div className="border-t border-gray-200 bg-gray-50 px-6 py-4">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-[2fr,1fr] md:items-end">
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-2 text-sm text-gray-600 md:flex-row md:items-center md:justify-between">
                <label className="inline-flex items-center gap-2">
                  ...
                  />
                  {translate('dashboard.eio.detail.createPlan')}
                </label>
                <div className="flex items-center gap-2">
                  <span>{translate('dashboard.eio.detail.ignoreForDays')}</span>
                  <input
                    type="number"
                    min="0"
                    value={ignoreDays}
                    onChange={(event) => setIgnoreDays(event.target.value)}
                    className="w-20 rounded border border-gray-300 px-2 py-1 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
                  />
                </div>
              </div>
            </div>
            <div className="flex flex-wrap justify-end gap-3">
              <button
                ...
                disabled={loading}
                className="inline-flex items-center rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100 disabled:opacity-60"
              >
